...
export async function pointsWatcher(path: string, shouldLog = false) {
  ...
  try {
    ...
    shouldLog && console.log(`[pointsWatcher] checking if file exists: ${path}`)
    ...
    if (exists) {
      shouldLog &&
        console.log(`[pointsWatcher] file already exists in database: ${path}`)
